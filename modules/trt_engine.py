...
class TRTModule:
    def __init__(self, weight: Union[str, Path]) -> None:
        """Initializes the TRTModule with the given model weight.

        Args:
            weight_path (Union[str, Path]): The path to the model weight file.
        """
    ...
    def __warm_up(self) -> None:
        """Performs a warm-up operation to initialize the engine."""
        if self.is_dynamic:
