...
def get_engine_backend():
    ...
    try:
        ...
        raise ValueError(
            "Unsupported BACKEND_MODULE. Please set it to either 'ONNX' or 'TENSORRT'.")
...
def run_coco_segmentation():
    ...
    downscaled_width, downscaled_height = int(
        downscaled_width_height[0]), int(downscaled_width_height[1])
    ...
    segmentation_thresholds = {
        "confidence_threshold": 0.25, "iou_threshold": 0.65}
    segmentation_results = postprocess_segmentation(
        inference_engine, downscaled_width, downscaled_height, background_removed_image.shape[
            :2],
...
def run_vehicle_segmentation():
    ...
    # Initialize inference engine and run segmentation
    inference_engine = InferenceEngine(image_tensor)
    # Select vehicles including bicycle, car, motorcycle, airplane, bus, train, truck
    vehicle_classes_from_coco = [2, 3, 4, 5, 6, 7, 8]
    segmentation_thresholds = {
        "confidence_threshold": 0.25, "iou_threshold": 0.65}
    segmentation_results = postprocess_segmentation(
        inference_engine, downscaled_width, downscaled_height, background_removed_image.shape[
            :2],
        vehicle_classes_from_coco, **segmentation_thresholds
    )
    # Process segmentation results
    annotations = []
    predicted_classes = set()
    vehicle_classes_from_coco = np.array(vehicle_classes_from_coco)
    for result in segmentation_results:
        contours = result["contours"]
        object_class = result["object_class"]
        mask_shape = result["mask_shape"]
        for contour in contours:
            ...
            predicted_classes.add(object_class - 1)
    segmentation_response = {"annotations": annotations,  "classes": np.array(CLASSES_GENERAL)[vehicle_classes_from_coco - 1].tolist(),
                             "predicted_classes": list(predicted_classes)}
    return jsonify(segmentation_response)
...
def run_person_segmentation():
    ...
    # Initialize inference engine and run segmentation
    inference_engine = InferenceEngine(image_tensor)
    person_classes_from_coco = [1]  # Person Class from COCO set
    segmentation_thresholds = {
        "confidence_threshold": 0.25, "iou_threshold": 0.65}
    segmentation_results = postprocess_segmentation(
        inference_engine, downscaled_width, downscaled_height, background_removed_image.shape[
    ...
    segmentation_response = {"annotations": annotations, "classes": ["person"],
                             "predicted_classes": [object_class - 1]}
    return jsonify(segmentation_response)
